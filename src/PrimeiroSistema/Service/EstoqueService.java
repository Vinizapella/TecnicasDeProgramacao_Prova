package PrimeiroSistema.Service;

import PrimeiroSistema.Model.Equipamento;
import PrimeiroSistema.Model.MotorEletrico;
import PrimeiroSistema.Model.PainelControle;
...
public class EstoqueService {
    ...
    private void pesquisarPorCodigo() {
        ...
        if (equipamento != null) {
            ...
            ui.mostrarEquipamento(equipamento);
        } else {
    ...
    private void movimentarEstoque() {
        ...
        if (tipoMovimentacao == 1) {
            if (adicionarEstoque(codigo, quantidade)) {
                ui.mostrarSucesso("Estoque adicionado! Quantidade atual: " + equipamento.getQuantidade());
            } else {
                ui.mostrarErro("Erro ao adicionar estoque!");
            }
        } else {
            if (retirarEstoque(codigo, quantidade)) {
                ui.mostrarSucesso("Estoque retirado! Quantidade atual: " + equipamento.getQuantidade());
            } else {
                ui.mostrarErro("Erro: Estoque insuficiente! Disponível: " + equipamento.getQuantidade());
            }
        }
    }
